...
const SERVICES = ["postgresql", "mysql", "mariadb", "redis", "valkey", "mongodb", "elasticsearch", "clickhouse", "rabbitmq", "memcached", "meilisearch", "ffmpeg", "ferretdb", "pict-rs", "soketi", "mqtt", "none"];
...
for (const file of files) {
  test(`schema: ${file}`, () => {
    ...
    assert.ok(a.docker.note === undefined || (typeof a.docker.note === "string" && a.docker.note.length > 0), `${file}: docker.note must be a non-empty string if present`);
